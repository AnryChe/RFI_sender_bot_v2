...
def gen_day_time_list(ins_df):  # Функция получения времени суток
    day_time_list = []
...
def time_format(ins_df):  # Функция обработки времени
    time_list = []
...
def data_from_excel(data_file_name):  # График ночных дежурств
    if socket.gethostname() == "Civil":
        readed_file_name = config.path + f'\\{data_file_name}.xlsx'
    else:
        readed_file_name = f'{data_file_name}.xlsx'
    data_df = pd.read_excel(readed_file_name, header=0, index_col=0)
    return data_df


def tn_insert(ins_df):
    tn_list = []
    schedule_df = data_from_excel('night_schedule')
...
def reformat_rfi(ins_df):  # Функция переформатирования списка инспекций под задачи статуса RFI
    ...
    for key in config.civ_sub_keys:  # Меняем название дисциплины для геодезических проверок
        full_df.loc[full_df['old_disc'].str.contains('civil') & full_df['Описание инспекций'].str.contains(
            key), 'Дисциплина'] = 'CIVIL/SUR'
...
def add_data_to_config(conf_username_dict, my_conf_name, my_config_file):
    my_config = configparser.ConfigParser()
    my_config.read(my_config_file)
    try:
