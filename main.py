...
from ex_ex import (InspectionsData, get_df_from_file, make_file_from_ref_df, reformat_rfi,
                   create_conf, add_data_to_config, read_conf, del_user_from_config, add_to_my_conf, clean_df,
...
# Объект бота
if platform.node() == "CIVIL":
    use_proxy = None
else:
    use_proxy = config.pythonanywhere_proxy
bot = Bot(token=config.inspection_bot_token, parse_mode='HTML',
          proxy=use_proxy)  # proxy='http://proxy.server:3128'
...
adm_keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
myboard = types.ReplyKeyboardMarkup(resize_keyboard=True)

buttons = ["Подписка", "Отписаться", "Телефон", "Мысль"]
adm_buttons = ["Раздача каналам", "Раздача лично", "Старт", "Телефон", "Мысль"]
myttons = ["Подписка", "Отписаться", "Телефон", "Номера", "Мысль", "Сотрудник", "Мемка"]

allboard.add(*buttons)
adm_keyboard.add(*adm_buttons)
myboard.add(*myttons)

mem_section = None
mem_name = None
emo_set = {"Ночные": emoji.emojize(":crescent_moon:"),  "Дневные": emoji.emojize(":sun:")}
    ...
    if message.chat.id > 0:
        if message.chat.id in config.flvb_list:
            ...
            await message.answer(
                ...
                'предоставляемую здесь информацию а также правильность распределения инспекций. Касательно исходных '
                'данных по инспекциям '
                ...
                'Телеграм установил ограничение по количеству сообщений в минуту для ботов. Имейте ввиду, что сообщения'
            ...
            await message.answer("Если Ваша фамилия в списке сотрудников, то после подписки сможете получать инспекции,"
                                 "которые были распределены Вам. \nРекомендуется, если количество инспекций в канале "
                                 "регулярно бывает более 6-8 штук. "
                                 "Выбор в любом случае теперь за Вами.\n"
                                 "Также не стоит пользоваться исключительно информацией в канале или от чат-бота, "
                                 "основной источник - официальная почта.\n"
                                 "Если при распределении инспекций Ваша фамилия не будет вписана (или вписана с "
                                 "ошибкой) в обработанном уведомлении -"
                                 " инспекция в личные сообщения не придет. Найти ее, как и все остальные инспекции "
                                 "можно в канале.\n"
                                 "Если не захотите далее получать шаблоны, отпишитесь от рассылки.")
            await asyncio.sleep(15)
            await message.answer(
                "Помимо появившихся внизу кнопок всегда можете воспользоваться в чате с ботом текстовыми "
                "командами: \n/start, Подписка, Отписаться, Телефон.\n"
                "Также Вы можете воспользоваться поиском номеров телефонов сотрудников Подрядчика, "
                "которые были указаны в уведомлениях об инспекции. Функция доступна для сотрудников компаний АГМК и "
                "Worley\n"
                "Если Вы сотрудник компаний, но не внесены в список, обратитесь к владельцу бота по ссылке ниже.\n"
                "Возможно в какой то момент при появлении новых сотрудников их номера будут отсутствовать, но "
                "справочник регулярно обновляется.\n"
                "Для поиска после нажатия кнопки Телефон (или просто текстовой команды в чат Телефон) введите "
                "несколько букв имени или фамилии.\n"
                ...
                "она позволит Вам блеснуть знаниями, что обесценит аргументы подрядчика и даст Вам время на поиск "
                "своих аргументов. \n"
                "Успехов в использовании, все просьбы и предложения пишите @AndreiChumak\n"
                "Кнопки внизу:",
                reply_markup=allboard)
        await bot.send_message(
            text=f"Пользователь с id {message.chat.id} с ником {message.from_user.username}, "
                 f"именем {message.from_user.full_name} зашел в чат",
            chat_id=config.group_id_list["my_id"], reply_markup=myboard)
        add_data_to_config({message.from_user.full_name: message.chat.id},
                           'unregistered_users', 'users_data.ini')
...
async def sstart_command(message: types.Message):
    if message.chat.id > 0:
        if message.chat.id in config.flvb_list:
            ...
            await message.answer("Привет! Я бот, который публикует шаблоны по дисциплинам инспекций МОФ-3\n"
                                 "Также можете сделать подписку для получения шаблонов распределенных инспекций.")
            await message.answer('<b>Дисклеймер.</b>\nЧат-бот и канал - личная разработка, к деятельности компании '
                                 'Worley не имеет никакого отношения. Автор не несет ответственности за '
                                 'предоставляемую здесь информацию а также правильность распределения инспекций. '
                                 'Касательно исходных данных по инспекциям '
                                 'рекомендую обращаться к ответственным лицам Подрядчика'
                                 'Продолжая использовать бот Вы даете согласие на обработку персональных данных'
                                 'Телеграм установил ограничение по количеству сообщений в минуту для ботов. '
                                 'Имейте ввиду, что сообщения будут приходить с задержкой')
            await asyncio.sleep(10)
            await message.answer("Если Ваша фамилия в списке сотрудников, то после подписки сможете получать инспекции,"
                                 "которые были распределены Вам. \nРекомендуется, если количество инспекций в канале "
                                 "регулярно бывает более 6-8 штук. "
                                 ...
                                 "основной источник - официальная почта.\n"
                                 "Если при распределении инспекций Ваша фамилия не будет вписана (или вписана с "
                                 "ошибкой) в обработанном уведомлении - инспекция в личные сообщения не придет. "
                                 "Найти ее, как и все остальные инспекции можно в канале.\n"
                                 "Если не захотите далее получать шаблоны, отпишитесь от рассылки.")
            await asyncio.sleep(15)
            await message.answer(
                "Помимо появившихся внизу кнопок всегда можете воспользоваться в чате с ботом текстовыми командами:"
                "\n/start, Подписка, Отписаться, Телефон.\n"
                "Также Вы можете воспользоваться поиском номеров телефонов сотрудников Подрядчика, "
                "которые были указаны в уведомлениях об инспекции. Функция доступна для сотрудников компаний АГМК "
                "и Worley\n"
                "Если Вы сотрудник компаний, но не внесены в список, обратитесь к владельцу бота по ссылке ниже.\n"
                "Возможно в какой то момент при появлении новых сотрудников их номера будут отсутствовать, "
                "но справочник регулярно обновляется.\n"
                "Для поиска после нажатия кнопки Телефон (или просто текстовой команды в чат Телефон) введите "
                "несколько букв имени или фамилии.\n"
                "Будут выведены все найденные совпадения\n"
                "Если закончились аргументы в споре с подрядчиком - воспользуйтесь функцией под названием Мысль, "
        ...
        if set_from_file['mess']:  # Проверяем, загрузился ли файл инспекций и соответствует ли он требуемому формату
            ...
            if message.chat.id != int(config.group_id_list["my_id"]):
                await bot.send_document(chat_id=config.group_id_list["my_id"], document=ref_doc,
                                        caption='Переформатированный файл для статуса', reply_markup=adm_keyboard)
            ...
            await message.answer(f"Заряжены инспекции из файла <b>{rec_file_name}</b>\n Для запуска раздачи "
                                 ...
                                 f"обратитесь к админу @AndreiChumak", reply_markup=adm_keyboard)
            await bot.send_message(text="дави кнопки\n", chat_id=config.group_id_list["my_id"],
                                   reply_markup=myboard)
...
async def inspect_command(message):
    global ready_insp_to_shot, emo_set
    if ready_insp_to_shot == 1:
        ...
        for uniq_discipline in clean_to_sign['sep_by_disc'].unique():
            ...
            for uniq_date in disc_inspections_df['Дата'].unique():
                ...
                for uniq_day_time in date_gen_inspections_df['Время дня'].unique():
                    e_set = emo_set.get(uniq_day_time)
                    dt_inspections_df = date_gen_inspections_df[date_gen_inspections_df['Время дня'] == uniq_day_time]
                    await bot.send_message(text=f'{e_set}{e_set}{e_set}{e_set}{e_set}{e_set}{e_set}',
            ...
            await bot.send_message(
                text=f'{emo_sub_set}Дисклеймер и инструкция по ссылке (рекомендуется к прочтению, просмотр по команде '
                     f'Старт):\n@mof3inspector_bot \nBe careful on inspections!',
                chat_id=config.group_id_list[uniq_discipline])
            if sur_ins:
                await bot.send_message(
                    text=f'{emo_sub_set}Дисклеймер и инструкция по ссылке (рекомендуется к прочтению, просмотр по '
                         f'команде Старт):\n@mof3inspector_bot \nBe careful on inspections!',
                    chat_id=config.group_id_list['___ГЕОДЕЗИЯ___'])
            sur_ins = False
            await asyncio.sleep(mess_time_out)
        ...
        if message.chat.id != int(config.group_id_list["my_id"]):
            await message.answer(
                text=f"<b>Раздача окончена в {datetime.datetime.strftime(now, '%H:%M  %d %m %Y')}</b>\n",
                reply_markup=adm_keyboard)
...
async def private_inspect_command(message):
    ...
    if ready_private_insp_to_shot == 1:
        ...
        await message.answer("Раздача инспекций в личку уже запущена. Дождитесь окончания раздачи, затем можно "
                             "будет зарядить инспекции вновь")
...
async def private_message_sender(cur_df_len, inspections_df, sub_text):  # функция шаблонов RFI
    ...
    for current_inspection in range(cur_df_len):  # Перебираем строки в датафрейме текущего вида работ
        ...
        if inspections_df['Примечание '][current_inspection] in str(reg_users_list.keys()):
            ...
            info_msg = await bot.send_message(
                text=f'Отправлено {sent_counter}-я инспекция '
                     f'{message_inspections.insp_number[message_inspections.insp_number.rfind("-") + 1:]}',
                chat_id=config.group_id_list['my_id'])
    ...
    if last_msg_id:
        await bot.delete_message(config.group_id_list['my_id'], last_msg_id)
    await bot.send_message(text=f'Отправлено в личку {sent_counter} инспекций', chat_id=config.group_id_list['my_id'],
                           reply_markup=myboard)
    await bot.send_message(text=f'Отправлено в личку {sent_counter} инспекций', chat_id=5709783583,
                           reply_markup=adm_keyboard)
    ...
    if message.chat.id > 0:
        ...
        if my_state_var == 0:
            ...
            await message.answer("    Для регистрации введите Вашу <b>фамилию</b> на русском языке (без инициалов).\n"
                                 ...
                                 "(за исключением случаев, когда раздача будет производиться до распределения "
                                 "инспекций, например, утренние инспекции в вечерней раздаче). \n"
                                 "По результатам тестирования возможно позднее будет добавлена дополнительная раздача."
                                 " Пока контроль за полнотой списка инспекций - за Вами! Претензии не принимаются. "
                                 "Только пожелания\n",
        ...
        await bot.send_message(
            text=f"Пользователь с id {message.chat.id} с ником {message.from_user.username}, "
                 f"именем {message.from_user.full_name} зашел в чат для подписки",
            chat_id=config.group_id_list["my_id"], reply_markup=myboard)
...
async def my_ini_setup(message: types.Message):
    if message.chat.id in config.flvb_list:
        try:
            ...
            await message.answer('стартовый список сотрудников компании создан', reply_markup=myboard)
...
async def add_company_user(message: types.Message):
    if message.chat.id == int(config.group_id_list["my_id"]):
        ...
        if my_state_var == 0:
            my_state_var = 11
            await message.answer(
                "    Для регистрации сотрудника введите <b>фамилию</b> на русском языке с инициалами.\n",
                reply_markup=types.ReplyKeyboardRemove())
        else:
...
async def mem_ini_setup(message: types.Message):
    if message.chat.id == int(config.group_id_list["my_id"]):
        try:
            read_conf('own_mem', 'users_data.ini')
            await message.answer('памятный список существует', reply_markup=myboard)
        except KeyError:
            create_conf({'test': 'one'}, 'own_mem', 'users_data.ini')
            add_to_my_conf('my_notes', 'users_data.ini')
            add_to_my_conf('my_links', 'users_data.ini')
            await message.answer('стартовый список памяток создан', reply_markup=myboard)


@dp.message_handler(Text(equals="Мемка"))  # Хэндлер на команду /start
async def choice_mem_section(message: types.Message):
    if message.chat.id == int(config.group_id_list["my_id"]):
        global my_state_var
        my_keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
        my_buttons = []
        sections = get_conf_section('users_data.ini')
...
async def add_my_mem(message: types.Message):
    global my_state_var
    if my_state_var == 0:
        get_conf_section('users_data.ini')
        my_state_var = 44
...
async def add_ee_handbook(message):
    try:
        ...
        add_data_to_config(kiok_dict, 'phone_numbers', 'users_data.ini')
        smr_data = gen_inspections_df['Отдел строительства']
        smr_dict = make_handbook(smr_data)
        add_data_to_config(smr_dict, 'phone_numbers', 'users_data.ini')
    ...
    if my_state_var == 1:
        ...
        if read_conf('company_users', 'users_data.ini').get(message.text.lower()):
            ...
            await bot.send_message(text=f"Пользователь {message.text} с id {message.chat.id} подписался на рассылку",
                                   chat_id=config.group_id_list["my_id"])
            add_data_to_config({message.text: message.chat.id}, 'registered_users',
                               'users_data.ini')
            ...
            await message.answer("Вашу фамилию не нашел в списке  сотрудников, повторите процедуру подписки, "
                                 "проверив верность ввода фамилий, или обратитесь в владельцу бота для проверки "
                                 "списка\n",
                                 reply_markup=keyboard)
            my_state_var = 0
    elif my_state_var == 5:
        ...
        add_data_to_config({user_short_name: user_full_name}, 'company_users',
                           'users_data.ini')
        ...
        add_data_to_config({mem_name: mem_text}, mem_section, 'users_data.ini')
